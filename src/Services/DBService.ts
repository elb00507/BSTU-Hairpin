...
export class DBService {
	...
	async getTypesGoods(): Promise<TTypesGoods> {
		const response = await fetch(
			this.domain +
				'is10_09?sSd_=0&sfil_n=2&svid_=3&sgr_l=160&sit_l=100&sgr_r=0&stst_=0&shead_=0&sadd_=5,104' +
				',85'
		);
	...
	async getGoodsByType(idGood: string): Promise<TGoodsResponse> {
		const response = await fetch(
			this.domain +
				'is10_09?sSd_=0&sfil_n=2&svid_=3&sgr_l=160&sit_l=110&sgr_r=0&stst_=0&shead_=0&sadd_=5,104' +
				',85' +
				',' +
				idGood
		);
